...
pcdcaap<-function(x,y){
  ...
  dsevere<-as.numeric(colSums(Diseasecat =="Severe"))
  dmoderate<-as.numeric(colSums(Diseasecat =="Moderate"))
  disease<-dsevere+dmoderate
  dnone<-as.numeric(colSums(Diseasecat=="Mild/No disease"))
  
  total<-dsevere+dmoderate+dnone
  psevere<-round((dsevere/total)*100,2)
  pmoderate<-round((dmoderate/total)*100,2)
  pnone<-round((dnone/total)*100,2)
  pdisease<-round((disease/total)*100,2)
  
  cat("Summary of the case definitions","\n")
  cat("SEVERE:" , psevere,"%","-", dsevere,"\n");
  cat("MODERATE:" , pmoderate,"%","-", dmoderate,"\n");  
  cat("MILD/NO DISEASE:" , pnone,"%","-", dnone,"\n"); 
  cat("  ","\n")
  cat("  ","\n")
  cat("Aggegating the cases with severe and moderate disease","\n")
  cat("DISEASE:" , pdisease,"%","-", disease,"\n");  
  cat("MILD/NO DISEASE:" , pnone,"%","-", dnone,"\n"); 
...
pefp<-function (x,y){
  ...
  disease<-as.numeric(colSums(category =="disease"))
  nodisease<-as.numeric(colSums(category=="No disease"))
  total<-disease+nodisease
  pdisease<-round((disease/total)*100,2)
  pnodisease<-round((nodisease/total)*100,2)
  cat("Summary of the case definitions","\n")
  cat("DISEAE:" , pdisease,"%","-", disease,"\n");
  cat("No DISEASE:" , pnodisease,"%","-", nodisease,"\n");  
}
